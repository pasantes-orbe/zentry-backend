...
module.exports = (sequelize: any, DataTypes: any) => {
    ...
    (CheckIn as any).associate = (models: any) => {
        // CORRECCIÓN: Usamos el nombre del modelo en minúsculas
        CheckIn.belongsTo(models.user, { foreignKey: 'id_guard', as: 'guardUser' });
        CheckIn.belongsTo(models.user, { foreignKey: 'id_owner', as: 'ownerUser' });
        CheckIn.belongsTo(models.country, { foreignKey: 'id_country', as: 'country' });
        // CORRECCIÓN: Usamos el nombre del modelo en minúsculas para referenciar checkout
        CheckIn.hasOne(models.checkout, { foreignKey: 'id_checkin', as: 'checkoutDetails' });
    };

    // Los métodos de instancia se pueden definir de esta manera más simple
    CheckIn.prototype.approve = async function() {
        this.check_in = true;
        await this.save();
        return this;
    };

    CheckIn.prototype.ownerConfirm = async function() {
        this.confirmed_by_owner = true;
        await this.save();
        return this;
    };
